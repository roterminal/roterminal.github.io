...
import { useRobloxApi, type RobloxUser } from "@/hooks/use-roblox-api";
...
export function SearchModal({ open, onClose, onSelectUser }: SearchModalProps) {
  ...
  const { loading, fetchUser, searchUsers } = useRobloxApi();
          ...
          <div className="p-2 max-h-60 overflow-y-auto">
            {results.map((user) => (
              ...
              >
                ...
                <span className="ml-auto text-xs text-muted-foreground font-mono">ID: {user.id}</span>
              </button>
            ))}
          </div>
        )}
